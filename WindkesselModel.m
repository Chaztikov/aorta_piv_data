...
R = 0.95000 ; % systemic per ipheral resistance in (mmHg/cmˆ3/ sec )
C = 1.0666 ; % systemic arterial compliance in (cmˆ3/mmHg)

% parameters for 3 element
R1 = 0.05;
R2 = 0.9000 ; % systemic peripheral resistance in (mmHg/cmˆ3/ sec )
C = 1.0666;
...
for n=1:cycle
    ...
    % Analytical solution for sistolic cycle
    ts=(n-1)*Tc:.01:(n-1)*Tc+Ts ;
    P0= P_ss+I0*Ts*R*(C*pi*R)/((Ts^2+C^2*pi^2*R^2));
    P_s= @(t)P0*exp(-((t-ts(1))/(R*C)))-I0*Ts*R*(C*pi*R*cos(pi*(t-ts(1))/Ts)-Ts*sin(pi*(t-ts(1))/-Ts))/(Ts^2+C^2*pi^2*R^2) ;
    P_sd=P_s(ts(end)) ; % IC for the diastolic phase
    
    % Analytical solution for diastolic cycle
    td=(n-1)*Tc+Ts:.01:n*Tc ;
    ...
    ylim([0 150])
    xlim([0 cycle*Tc])
    title('Aortic Blood Pressure(Numerical Analysi s? 2Element WM)')
    ylabel('Pressure(mmHg)')
    xlabel('time(s)')
    ...
    if(n==1)
        ...
        t3=t_m3;
        P3=P_m3; % Numerical solution for 3 element WM
    end
    
end

%%Analysi s of Model with Various Initionl Conditions
% Modelling Blood flow into Aorta
squared=@(t)(square(pi*(t-((Tc/2)-Ts))/Ts,100*(2*Ts-(Tc/2))/Tc)+1)/2;
I_estimate=@(t)I0*squared(t).*sine(t); %% Estimated perodic Blood flow model
...
%%Comparision of WM and analytical Solution
figure(4)
hold on
