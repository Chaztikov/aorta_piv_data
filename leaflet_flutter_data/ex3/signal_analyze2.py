...
for fname0 in fnames[:1]:
    ...
    varfunc.shape
    meanfunc.shape
    ub = varfunc / np.sqrt(2)
    lb = varfunc / np.sqrt(2)

    (data[:]==0).sum(axis=1)
    plt.plot(times,data.T,'k.',alpha=0.9);plt.show()
    plt.plot(times,cdata.T,'k.',alpha=0.9);plt.show()

    plt.plot(meanfunc.T,'k.',alpha=0.9);plt.show()


    plt.figure(figsize=(24,12))
    plt.fill_between(times,cdata+lb,cdata+ub,'k.',alpha=0.8);
    plt.grid()
    plt.xlabel('Time')
    plt.ylabel('Area')
